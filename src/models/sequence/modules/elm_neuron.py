...
def create_overlapping_window_indices(
    ...
    overlapping_indices = (torch.arange(num_windows).unsqueeze(1) * stride_size
...
class ELM(jit.ScriptModule):
    ...
    def __init__(
        ...
        self.tau_b_value, self.w_s_value = tau_b_value, w_s_value
    ...
    @jit.script_method
    def dynamics(self, x, b_prev, m_prev, w_s, kappa_b, kappa_m, kappa_lambda):
    ...
    @jit.script_method
    def forward(self, X):
        X = X.permute(1, 0, 2).contiguous()  # new add for alignment dimention
        ...
        for t in range(T):
            ...
            outputs.append(y_t)
        # return torch.stack(outputs, dim=-2) # before
        return torch.stack(outputs, dim=0)  # current

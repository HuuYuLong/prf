...
class StackedRNN(SequenceModule):
    ...
    def forward(self, inputs, *args, state=None, **kwargs):
        ...
        for rnn, prev_state, lin, dropout in zip(self.rnn_layers, prev_states, self.lin_layers, self.dropout_layers):
            ...
            # If dropout, only apply to the outputs_202411_202504 of RNNs that are not the last one (like torch's LSTM)
            outputs = dropout(outputs)

            z = lin(outputs)

            if self.skip_connections:
                # If skip connections, add the outputs_202411_202504 of all the RNNs to the outputs_202411_202504
                out += z
                # Feed in the outputs_202411_202504 of the previous RNN, and the original inputs to the next RNN

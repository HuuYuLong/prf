...
class SequenceResolutionCollateMixin(DefaultCollateMixin):
    ...
    def _collate_callback(cls, x, resolution=None):
        if resolution is None:
            ...
            rhs = " ".join([f"res{i}" for i in range(n_resaxes)]) + " b " + " ".join(
                [f"l{i}" for i in range(n_resaxes)]) + " ..."
...
class TBPTTDataLoader(torch.utils.data.DataLoader):
    ...
    def __iter__(self):
        for batch in super().__iter__():
            ...
            pad = lambda x, val: torch.cat([x.new_zeros((x.shape[0], self.overlap_len - 1, *x.shape[2:])) + val, x],
                                           dim=1)
            x = pad(x, self.zero)
            y = pad(y, 0)

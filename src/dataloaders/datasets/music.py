...
def download_all_data(path):
    ...
    for item in ITEM_LIST:
        os.system(
            "wget -r -H -nc -nH --cut-dir=1 -A .ogg -R *_vbr.mp3 -e robots=off -P " + path + " -l1 'http://archive.org/download/" + item + "'")
...
class _Music():
    def __init__(
        ...
        self.train_X, self.val_X, self.test_X, self.train_y, self.val_y, self.test_y = self.make_x_y(self.train,
                                                                                                     self.val,
                                                                                                     self.test)
    ...
    def make_x_y(self, train, val, test):
        ...
        if not self.samplernn_proc:
            ...
            if not self.discrete_input:
                train_X, val_X, test_X = torch.roll(mu_law_decode(train_y), 1, 1), torch.roll(mu_law_decode(val_y), 1,
                                                                                              1), torch.roll(
                    mu_law_decode(test_y), 1, 1)

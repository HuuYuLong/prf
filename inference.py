...
def train(config):
    ...
    sum_all = 0.
    layer_num = 1
    for modlue in model.model.layers:
        tn_fr = modlue.layer.TN_fr / modlue.layer.count
        sn_fr = modlue.layer.SN_fr / modlue.layer.count
        print(f"TN_fr_{layer_num}", tn_fr)
        print(f"SN_fr_{layer_num}", sn_fr)
        # print(modlue.layer.count)
        # print(modlue.save_firerate)
        layer_num += 1
        sum_all += (tn_fr + sn_fr)

    print("the avg.")
    print("avg_fr", sum_all / (2*layer_num))
